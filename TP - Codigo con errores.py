...
class Servidor():
    ...
    def creoGrafico(self,nivel):
        ...
        plt.title('Uso del servidor ' + str(self.numeroServidor + 1))
...
class Nivel:
            ...
            if eventoArribo.prioridad == False:
                for i in range(len(self.colas)):
                    ...
                    if comparador==genteEnCola and self.nivel==1:
                        listaAux=[0,1]
                        indice=random.choice(listaAux)
                self.colas[indice].agregarArribo(eventoArribo)
            else:
    ...
    def partida(self,eventoPartida):
        ...
        for i in range(len(self.servidores)):
            print(self.servidores[i].estado)
        ...
        listaAux = []
        for i in range(len(self.colas)):
            if self.colas[i].dameTuLongitud() != 0:
                listaAux.append(self.colas[i])
        if len(listaAux) != 0:                              #Si las colas no estan vacias
    ...
    def desocupoServidor(self, partida):
        for i in range(len(self.servidores)):
            if self.servidores[i].partida.tiempoPartida == partida.tiempoPartida:
    ...
    def dameTusDatos(self):

        for i in range(len(self.servidores)):
            self.servidores[i].dameTusDatos(Simu.relojSimulacion)

        for j in range(len(self.colas)):
            self.colas[j].dameTusDatos(Simu.relojSimulacion)
...
class Simulador():
    ...
    def informe(self):
        ...
        for i in range(len(self.niveles)):
            ...
            self.niveles[i].dameTusDatos()
            print("Demora promedio en el nivel:",i+1," : ",self.niveles[i].totalDeDemoras/self.cantDeDemorados)
            print("-------------------------------------------")
